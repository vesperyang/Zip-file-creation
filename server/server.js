...
app.get("/status/:fileName", async (req, res) => {
  ...
  if (compressionStatus[fileName] === "completed") {
    const presignedUrl = await generatePresignedUrl(fileName);
    res.json({ status: "completed", download_link: presignedUrl });
  } else if (compressionStatus[fileName] === "waiting") {
...
app.post("/upload", upload.single("files"), async (req, res) => {
  ...
  try {
    ...
    compressionStatus[originalFileName] = "waiting";

    compressFileInBackground(originalFileName);

    res.json({ message: "File compression has started. The download link will be available shortly.",fileName:originalFileName });
  } catch (err) {
...
// Function to compress the file in the background
async function compressFileInBackground(fileName) {
  console.log(`***compressFileInBackground: ${fileName}`);
  const fileHash = await generateHash(`./uploads/${fileName}`);

  const compressionProcess = spawn("bzip2", ["./uploads/" + fileName]);

  compressionProcess.on("close", async (code) => {
    if (code === 0) {
      console.log("File compression completed successfully.");


      await saveToRedis(fileHash, fileName);
      await uploadFileToS3(fileName);
      compressionStatus[fileName] = "completed";

    } else {
      console.error("Error: File compression failed");

      compressionStatus[fileName] = "failed";
    }
  });

  compressionProcess.on("error", (err) => {
    console.error("Error starting compression process:", err);
